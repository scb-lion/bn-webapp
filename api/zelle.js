...
module.exports = async (req, res) => {
  ...
  if (req.method === 'GET') {
    return json(res, 200, payload(user));
  }
  ...
  if (req.method === 'POST') {
    ...
    const recipients = (user.zelleRecipients || []).slice();
    ...
    await users.updateOne({ _id: user._id }, { $set: { zelleRecipients: recipients, updatedAt: new Date() } });
  ...
  if (req.method === 'DELETE') {
    ...
    const recipients = (user.zelleRecipients || []).filter((r) => String(r.id) !== id);
    await users.updateOne({ _id: user._id }, { $set: { zelleRecipients: recipients, updatedAt: new Date() } });
  ...
  if (req.method === 'PATCH') {
    ...
    if (body.defaultAccountId !== undefined) {
      ...
      if (id && !(user.accounts || []).some((a) => String(a.id) === id)) {
        return json(res, 400, { error: 'Default account is not one of your accounts' });
      }
      zelle.defaultAccountId = id;
    }
    await users.updateOne({ _id: user._id }, { $set: { zelle: zelle, updatedAt: new Date() } });
